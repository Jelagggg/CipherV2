...
if (isset($_POST["addmaj"])){
  if (!empty($_POST["nomaj"] and $_POST["contenup"])){
    $stmt = $db->prepare("INSERT INTO updates (name, content, byname) VALUES (?, ?, ?) ");
    $stmt->bind_param("sss", $name, $content, $byname);
    $name = htmlspecialchars($_POST["nomaj"]);
    $content = $_POST["contenup"];
    $byname = htmlspecialchars($_SESSION["username"]);
    ...
    $comment = "Nouvelle mise à jour : ".$_POST["nomaj"]." par ".$_SESSION["username"].".";
    $heure = strftime('%d %B %Y à %H:%M',strtotime("+6 hours")); 
    $rank = "2";
    $status = "success";
    $stmtl->execute();
    $stmtl->close();
    $_POST['success'] = 'MàJ créée avec succès.';
    $stmt->close();
    }else{
          ...
          <div class="col-md-12">
              <div class="card">
                <div class="card-header card-header-primary">
                  ...
                  <p class="card-category">Créer une mise à jour</p>
                </div>
                <div class="card-body">
                  <form method="POST">
                    <div class="row">
                      <div class="col-md-12">
                        <div class="form-group">
                          <label class="bmd-label-floating">Nom de la MàJ</label>
                          <input type="text" class="form-control" name="nomaj">
                        </div>
                      </div>
                      </div>
                    <div class="row">
                      <div class="col-md-12">
                        <div class="form-group">
                          <div class="form-group">
                            ...
                            <textarea class="form-control" rows="20" name="contenup"></textarea>
                          </div>
                        </div>
                      </div>
                    </div>
                    <button type="submit" name="addmaj" class="btn btn-success pull-right"><i class="fas fa-check"></i>  Créer</button>
