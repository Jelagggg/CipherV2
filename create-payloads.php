...
if (isset($_POST["addpayload"])){
  if (!empty($_POST["nompay"] and $_POST["cate"] and $_POST["contenup"])){

    $getaccess = "SELECT token FROM users WHERE username = '".$_SESSION["username"]."'";
    if($result = mysqli_query($db, $getaccess)){
    if(mysqli_num_rows($result) > 0){
        while($rowa = mysqli_fetch_array($result)){

    $sqld = "SELECT * FROM payloadscate WHERE name = '".$_POST["cate"]."' AND token = '".$rowa["token"]."'";
  ...
  $comment = "Nouveau payload : ".$_POST["nompay"]." par ".$_SESSION["username"].".";
  $heure = strftime('%d %B %Y à %H:%M',strtotime("+6 hours")); 
  $rank = "1";
  $status = "success";
  $stmtl->execute();
  $stmtl->close();
  $_POST['success'] = 'Payload crée avec succès.';
  $stmt->close();
  }else{
          ...
          <div class="col-md-12">
              <div class="card">
                <div class="card-header card-header-primary">
                  ...
                  <p class="card-category">Créer un payload</p>
                </div>
                <div class="card-body">
                  <form method="POST">
                    <div class="row">
                      <div class="col-md-6">
                        <div class="form-group">
                          ...
                          <input type="text" class="form-control" name="nompay">
                        </div>
                      </div>
                      <div class="col-md-6">
                        <div class="form-group">
                    ...
                    <button type="submit" name="addpayload" class="btn btn-success pull-right"><i class="fas fa-check"></i>  Sauvegarder</button>
